...
function unlockHandler(data) {
  if (doorStatus == "closed") {
    ...
    try {
      if (response.token && socketMap[response.token]) {
        socketMap[response.token].write(JSON.stringify(response));
        if (!lotteDebug) socketMap[response.token].write("\n");
      } else {
        logger.error("not found socket: ", response);
      }
    } catch (error) {
...
var server = net.createServer(function (socket) {
  ...
  socket.on("data", (data) => {
    ...
    try {
      ...
      logger.log("Received: ", msg);
      if (msg.type) {
        if (msg.type == "unLock" && msg.token) {
          if (!socketMap[msg.token]) socketMap[msg.token] = socket;
          else logger.log("token is exist: ", msg.token);
        }
        eventHandler[msg.type](msg);
      }
    } catch (error) {
